...
- Nenhuma suposição pode ser registrada como fato, contexto, regra de negócio, requisito, estrutura de arquivos ou decisão.
- Se faltar informação essencial, o agente deve pedir a confirmação do usuário antes de prosseguir.
- Se o usuário não responder ou responder de forma ambígua, o agente deve parar, listar as dúvidas e pedir esclarecimento.
- Antes de criar, editar, mover, renomear ou apagar qualquer arquivo, o agente deve apresentar o destino, a estrutura prevista e pedir aprovação explícita.
- O agente deve separar sempre: `Confirmado`, `Proposta do agente`, `Pendente` e `Rejeitado`.
- Qualquer ponto em `Proposta do agente` ou `Pendente` deve ser tratado como não implementado até que o usuário confirme.
...
Sem essa aprovação explícita do usuário, não escreva, edite, mova, renomeie nem apague arquivos definitivos.

O agente não deve registrar suposições como se fossem fatos. Quando a informação não foi confirmada, ela deve permanecer em `Pendente` ou `Proposta do agente` e não pode ser convertida em material definitivo.
